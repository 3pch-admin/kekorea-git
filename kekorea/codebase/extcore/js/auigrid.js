...
function headerMenuSelectHandler(event, ui) {
	...
	case "h_item_4": // 현재 칼럼 숨기기
		...
		$("#h_item_ul span.ui-icon[data=" + currentDataField + "]").removeClass("ui-icon-check").addClass("ui-icon-blank");
		break;
	case "h_item_6": // 모든 칼럼 보이기
		AUIGrid.showAllColumns(myGridID);
		$("#h_item_ul span.ui-icon[data]").addClass("ui-icon-check").removeClass("ui-icon-blank");
		break;
	default: // 헤더 보이기 / 숨기기
		...
		if (typeof dataField != "undefined") {
			...
			if (checked) {
				...
				kids.removeClass("ui-icon-check").addClass("ui-icon-blank");
			} else {
...
/**
 * LAZY LOAD 공통 리스트 페이지에서 모두 밖으로 제외
 */
let last = false;
...
// document.getElementById("lastNum").value = Number(lastNum)
// + data.list.length;
// }
