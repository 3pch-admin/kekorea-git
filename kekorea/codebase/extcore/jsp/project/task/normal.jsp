...
<%
	...
	String tProg = (String) request.getParameter("tProg") == null ? "0" : request.getParameter("tProg");
%>
	...
	const columns = [ {
		...
		renderer : {
			...
			jsCallback : function(rowIndex, columnIndex, value, item) {
				...
				const url = getCallUrl("/output/view?oid=" + oid+"&tProg=<%=tProg%>");
				popup(url, 1800, 750);
			}
		},
	}, {
...
<%-- 		AUIGrid.setGridData(myGridID, <%=ProjectHelper.manager.jsonAuiOutput(poid, toid)%>); --%>
	...
	function create() {
		...
		const url = getCallUrl("/output/create?toid=" + toid + "&poid=" + poid+"&tProg=<%=tProg%>");
		popup(url, 1600, 1000);
	}
