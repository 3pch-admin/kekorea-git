...
<%} %>
		...
		<%if( dto.getPoid() != null && dto.getPoid().length() > 0 ){  %>
		<!-- 결재이력 -->
...
<script type="text/javascript">
	...
	function _receive() {
		...
		call(url, params, function(data) {
			...
			if (data.result) {
				opener.loadGridData();
				opener.parent.parent.document.getElementById("_receive").innerHTML = data._receive;
				self.close();
			} else {
				closeLayer();
			}
	...
	function _unagree() {
		...
		call(url, params, function(data) {
			...
			if (data.result) {
				opener.loadGridData();
				opener.parent.parent.document.getElementById("_agree").innerHTML = data._agree;
				self.close();
			} else {
				closeLayer();
			}
	...
	function _reject() {
		...
		call(url, params, function(data) {
			...
			if (data.result) {
				opener.loadGridData();
				opener.parent.parent.document.getElementById("_approval").innerHTML = data._approval;
				self.close();
			} else {
				closeLayer();
			}
	...
	function _approval() {
		...
		call(url, params, function(data) {
			...
			if (data.result) {
				opener.loadGridData();
// 				opener._refresh(data._approval);
				opener.parent.parent.document.getElementById("_approval").innerHTML = data._approval;
				self.close();
			} else {
				closeLayer();
