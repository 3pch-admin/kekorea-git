...
if (per instanceof ApprovalContract) {
	...
	if ("PART_CODE".equals(contract.getContractType())) {
%>
<!-- 부품 코드 -->
...
<table class="view-table">
	...
	for (Map<String, Object> map : list) {
		boolean isLib = (boolean) map.get("isLib");
		if (!isLib) {
	%>
	<tr>
		<td class="indent5">
		...
		<td class="center"><%=map.get("dwg_no")%></td>
...
<%
} else if ("EPMDOCUMENT".equals(contract.getContractType())) {
%>
<!-- 일괄결재 도면 -->
<table class="button-table">
	<tr>
		...
		<td class="right">
			...
			<!-- 				<input type="button" value="도면승인 요청서 다운로드" title="도면승인 요청서 다운로드" class="green" onclick=""> -->
			<!-- 			</a> -->
...
<%
} else if ("NUMBERRULE".equals(contract.getContractType())) {
%>
<table class="button-table">
	<tr>
		<td class="left">
		...
		<td class="right">
			<input type="button" value="도번승인 요청서 다운로드" title="도번승인 요청서 다운로드" class="green" onclick="kekNumber();">
		</td>
	</tr>
</table>
...
<%
} else if ("OUTPUT".equals(contract.getContractType())) {
%>
<!-- 작번 산출물 문서 -->
<table class="button-table">
	<tr>
...
if (per instanceof PartListMaster) {
	...
	Vector<Map<String, Object>> secondarys = ContentUtils.getSecondary(mm);
%>
<!-- 수배표 -->
<table class="button-table">
	<tr>
...
<table class="view-table">
	...
	<tr>
		<td class="indent5">
			<a href="javascript:_detail('<%=roid%>');"><%=requestDocument.getName()%></a>
		</td>
...
<script type="text/javascript">
	function _detail(oid) {
		...
		if (oid.indexOf("Project") > -1) {
			...
			url += "/requestDocument/view?oid=" + oid;
			popup(url, 1700, 800);
			...
			url += "/output/view?oid=" + oid + "&tProg=0";
			popup(url, 1600, 800);
		}else if (oid.indexOf("Meeting") > -1) {
			url += "/meeting/view?oid=" + oid + "&tProg=0";
			popup(url, 1600, 800);
		}
	}
	function update() {
		...
		const url = getCallUrl("/epm/update?oid=<%=oid%>");
		document.location.href = url;
	}
	
	function kekNumber() {
		const oid = "<%=oid%>";
		const url = getCallUrl("/workspace/kekNumber?oid=" + oid);
		openLayer();
		call(url, null, function(data) {
			if (data.result) {
				const n = data.name;
				document.location.href = '/Windchill/extcore/jsp/common/FileDownload.jsp?fileName=' + n + '&originFileName=' + n;
			}
			closeLayer();
		}, "GET");
	}
