	...
	<tr>
		...
		<td class="right">
			...
			<%
				if (isAdmin) {
			%>
			<input type="button" value="삭제" title="삭제" onclick="_delete();" class="red">
			<%
				}
			%>
			<%
			...
			%>
			<input type="button" value="개정" title="개정" onclick="update('revise');" style="background-color: orange;">
			<%
				//}
			%>
...
<script type="text/javascript">
	function _delete() {
		...
		const url = getCallUrl("/workOrder/delete?oid=" + oid);
		openLayer();
