	...
	<form>
		...
		<table class="search-table">
			...
			<tr>
				...
				<th>작성일</th>
				<td class="indent5">
					<input type="text" name="createdFrom" id="createdFrom" class="width-100" value="<%=before%>">
					~
					<input type="text" name="createdTo" id="createdTo" class="width-100" value="<%=end%>">
		...
		<table class="button-table">
			<tr>
				...
				<td class="right">
					...
					<input type="button" value="개정" title="개정" style="background-color: orange;" onclick="revise();">
					<%-- <%
						if(isSupervisor) {
					%> --%>
		...
		<div id="grid_wrap" style="height: 655px; border-top: 1px solid #3180c3;"></div>
		...
		<%=classificationWritingDepartments%>
			...
			function register() {
				...
				for (let i = 0; i < checkedItems.length; i++) {
					...
					if (item.state !== "작업 중") {
						alert("작업 중인 도번만 결재를 할 수 있습니다.");
						return false;
					}
					arr.push(item);
				}
				const url = getCallUrl("/numberRule/register");
				let p = popup(url, 1475, 650);
				p.data = arr;
			}
			...
			function loadGridData() {
				...
				const field = [ "number", "name", "lotNo", "unitName", "size", "state", "writtenDocuments_code", "creatorOid", "createdFrom", "createdTo", "classificationWritingDepartments_code" ];
