...
//					}
	...
	@Override
	public void dataSave(Map<String, ArrayList<Map<String, String>>> params) throws Exception {
		ArrayList<Map<String, String>> editRows = (ArrayList<Map<String, String>>) params.get("editRows");
		Transaction trs = new Transaction();
		try {
			trs.start();
			...
			for (Map<String, String> editRow : editRows) {
				String hoid = editRow.get("history");
				String oid = editRow.get("oid");
				String key = editRow.get("key");
				String value = editRow.get("value");

				History history = (History) CommonUtils.getObject(hoid);
				if (StringUtils.isNull(oid)) {
					HistoryValue v = HistoryValue.newHistoryValue();
					v.setValue(value);
					v.setHistory(history);
					v.setDataField(key);
					PersistenceHelper.manager.save(v);
				} else {
					HistoryValue v = (HistoryValue) CommonUtils.getObject(oid);
					v.setValue(value);
					PersistenceHelper.manager.modify(v);
				}
			}
