...
//		QuerySpecUtils.toLikeAnd(query, idx_p, Project.class, Project.KEK_NUMBER, kekNumber);
	...
	public JSONArray view(String oid) throws Exception {
		...
		QuerySpecUtils.toInnerJoin(query, History.class, Project.class, "projectReference.key.id",
				WTAttributeNameIfc.ID_NAME, idx, idx_p);
		QuerySpecUtils.toEqualsAnd(query, idx, History.class, WTAttributeNameIfc.ID_NAME,
				history.getPersistInfo().getObjectIdentifier().getId());
		QueryResult qr = PersistenceHelper.manager.find(query);

		Object[] obj = (Object[]) qr.nextElement();
		History his = (History) obj[0];
		Project project = (Project) obj[1];
		Map<String, Object> dataMap = new HashMap<>();
		dataMap.put("kekNumber", project.getKekNumber());
		dataMap.put("keNumber", project.getKeNumber());
		...
		dataMap.put("install", project.getInstall() != null ? project.getInstall().getName() : "");
		dataMap.put("pdate", project.getPDate());
		dataMap.put("tuv", his != null ? his.getTuv() : "");
		dataMap.put("poid", project.getPersistInfo().getObjectIdentifier().getStringValue());
		dataMap.put("oid", his != null ? his.getPersistInfo().getObjectIdentifier().getStringValue() : "");

		ArrayList<HistoryValueLink> data = getValues(his);
		for (HistoryValueLink link : data) {
			HistoryValue historyValue = link.getValue();
			String dataField = historyValue.getDataField();
			String value = historyValue.getValue();
...
//		map.put("curPage", pager.getCpage());
	...
	public JSONArray tab(Project project, ArrayList<SpecCode> fixedList) throws Exception {
		...
		query.appendWhere(sc, new int[] { idx, idx_p });
		query.appendAnd();
		sc = new SearchCondition(History.class, "projectReference.key.id", "=",
		...
		if (qr.hasMoreElements()) {
			...
			for (SpecCode fix : fixedList) {
				...
				mergedList.put("key", fix.getName());
				mergedList.put("hoid", history.getPersistInfo().getObjectIdentifier().getStringValue());
				...
				if (result.hasMoreElements()) {
					...
					mergedList.put("value", historyValue.getValue());
					mergedList.put("oid", historyValue.getPersistInfo().getObjectIdentifier().getStringValue());
				}
				list.add(mergedList);
			}
