...
import e3ps.project.output.OutputDocumentMasterLink;
...
//		int day = ca.get(Calendar.DATE);
	...
	public JSONArray versionHistory(WTDocument document) throws Exception {
		...
		while (result.hasMoreElements()) {
			...
			map.put("modifier", dd.getModifierFullName());
			map.put("modifiedDate_txt", CommonUtils.getPersistableTime(dd.getModifyTimestamp()));
	...
	public JSONArray jsonAuiProject(String oid) throws Exception {
		...
		if (oid.indexOf("WTDocument") > -1) {
			...
			E3PSDocument document = (E3PSDocument) CommonUtils.getObject(oid);
			QueryResult result = PersistenceHelper.manager.navigate(document.getMaster(), "output",
					OutputDocumentMasterLink.class);
			while (result.hasMoreElements()) {
				Output output = (Output) result.nextElement();
				Project project = output.getProject();
				Map<String, String> map = new HashMap<>();
			...
			result.reset();
			PRJDocument prj = (PRJDocument) CommonUtils.getObject(oid);
			result = PersistenceHelper.manager.navigate(prj, "output", OutputDocumentLink.class);
			while (result.hasMoreElements()) {
				Output output = (Output) result.nextElement();
				Project project = output.getProject();
				Map<String, String> map = new HashMap<>();
	 ...
	 * 제작사양서 조회 함수
	 */
	public Map<String, Object> only(Map<String, Object> params) throws Exception {
		Map<String, Object> map = new HashMap<>();
		ArrayList<DocumentDTO> list = new ArrayList<>();
...
//			/Default/문서/프로젝트/제작사양서
			Folder folder1 = FolderTaskLogic.getFolder(loc1, CommonUtils.getPDMLinkProductContainer());
			Folder folder2 = FolderTaskLogic.getFolder(loc2, CommonUtils.getPDMLinkProductContainer());
			if (query.getConditionCount() > 0)
				query.appendAnd();
			int f_idx = query.appendClassList(IteratedFolderMemberLink.class, false);
			ClassAttribute fca = new ClassAttribute(IteratedFolderMemberLink.class, "roleBObjectRef.key.branchId");
			...
			Folder folder = FolderTaskLogic.getFolder(DocumentHelper.SPEC_OLD_ROOT,
