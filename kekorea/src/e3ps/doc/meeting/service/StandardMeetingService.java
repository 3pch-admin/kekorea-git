...
public class StandardMeetingService extends StandardManager implements MeetingService {
	...
	public void create(MeetingDTO dto) throws Exception {
		...
		try {
			...
			Meeting meet = Meeting.newMeeting();
			meet.setNumber(MeetingHelper.manager.getNextNumber());
			meet.setName(name);
			meet.setOwnership(CommonUtils.sessionOwner());
			...
			if (!StringUtils.isNull(tiny)) {
				...
				meet.setTiny(meetingTemplate);
			}

			meet.setDocType(DocumentType.toDocumentType("$$Meeting"));

			Folder folder = FolderTaskLogic.getFolder(MeetingHelper.LOCATION, CommonUtils.getPDMLinkProductContainer());
			FolderHelper.assignLocation((FolderEntry) meet, folder);

			meet = (Meeting) PersistenceHelper.manager.save(meet);
			meet = (Meeting) PersistenceHelper.manager.refresh(meet);

			for (int i = 0; secondarys != null && i < secondarys.size(); i++) {
				String cacheId = (String) secondarys.get(i);
				File vault = CommonContentHelper.manager.getFileFromCacheId(cacheId);
				ApplicationData applicationData = ApplicationData.newApplicationData(meet);
				applicationData.setRole(ContentRoleType.SECONDARY);
				PersistenceHelper.manager.save(applicationData);
				ContentServerHelper.service.updateContent(meet, applicationData, vault.getPath());
			}
			...
			for (Map<String, String> addRow9 : addRows9) {
				...
				MeetingProjectLink link = MeetingProjectLink.newMeetingProjectLink(meet, project);
				PersistenceHelper.manager.save(link);

				// 산출물
				Output output = Output.newOutput();
				output.setName(meet.getName());
				output.setLocation(meet.getLocation());
				output.setTask(t);
				output.setProject(project);
				output.setDocument(meet);
...
//				ProjectHelper.service.calculation(project);
			...
			if (approvalRows.size() > 0) {
				WorkspaceHelper.service.register(meet, agreeRows, approvalRows, receiveRows);
			}
	...
	public void delete(String oid) throws Exception {
		...
		try {
			...
			WorkspaceHelper.manager.deleteAllLines(meeting);
	...
	public void update(MeetingDTO dto) throws Exception {
		...
		try {
			...
			if (isApproval) {
				...
				mm = WorkspaceHelper.manager.getMaster(meeting);
			}

			meeting = (Meeting) PersistenceHelper.manager.refresh(meeting);
			...
			newMeeting = (Meeting) PersistenceHelper.manager.refresh(newMeeting);

			// WorkspaceHelper.manager.deleteAllLines(conSheet);

			if (!isApproval) {
				if (mm != null) {
					String n = WorkspaceHelper.manager.getName(newMeeting);
					mm.setPersist(newMeeting);
					mm.setName(n);
			...
			if (approvalRows.size() > 0) {
				WorkspaceHelper.service.register(newMeeting, agreeRows, approvalRows, receiveRows);
			}
		...
		try {
			...
			for (String oid : arr) {
				...
				while (qr.hasMoreElements()) {
					...
					MeetingProjectLink link = (MeetingProjectLink) o[0];
					PersistenceHelper.manager.delete(link);
				}
