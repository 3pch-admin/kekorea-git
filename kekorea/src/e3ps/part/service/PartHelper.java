...
//		WTPart part = (WTPart) CommonUtils.getObject(oid);
	...
	/**
	 * 부품 일괄등록 PLM 있는 데이터 가져오기
	 */
	public Map<String, Object> batchValidatorNumber(String number) throws Exception {
		// YCODE 입력으로 있는지 확인 후 IBA값 리턴..
		Map<String, Object> map = new HashMap<String, Object>();
		QuerySpec query = new QuerySpec();
		int idx = query.appendClassList(WTPart.class, true);
		QuerySpecUtils.toIBAEqualsAnd(query, WTPart.class, idx, "PART_CODE", number);
		...
		if (result.hasMoreElements()) {
			Object[] obj = (Object[]) result.nextElement();
			WTPart part = (WTPart) obj[0];
			String name = IBAUtils.getAttrValue(part, "NAME_OF_PARTS");
			String maker = IBAUtils.getAttrValue(part, "MAKER");
			String spec = IBAUtils.getAttrValue(part, "DWG_NO");
			String unit = IBAUtils.getAttrValue(part, "STD_UNIT");
			int price = IBAUtils.getIntegerValue(part, "PRICE");
			String currency = IBAUtils.getAttrValue(part, "CURRNAME");
			String customer = IBAUtils.getAttrValue(part, "CUSTNAME");
	 ...
	 * IBA PART_CODE
	 */
	public WTPart getPartByNumber(String partNo) throws Exception {
		QuerySpec query = new QuerySpec();
		int idx = query.appendClassList(WTPart.class, true);
//		int idx_m = query.appendClassList(WTPartMaster.class, false);
...
//		QuerySpecUtils.toInnerJoin(query, WTPart.class, CWTPartMaster.class, "masterReference.key.id",
//				WTAttributeNameIfc.ID_NAME, idx, idx_m);
