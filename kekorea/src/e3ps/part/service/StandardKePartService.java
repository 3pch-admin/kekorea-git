...
public class StandardKePartService extends StandardManager implements KePartService {
	...
	@Override
	public void modify(HashMap<String, List<KePartDTO>> dataMap) throws Exception {
		List<KePartDTO> addRows = dataMap.get("addRows");
		Transaction trs = new Transaction();
		try {
			trs.start();
			...
			for (KePartDTO dto : addRows) {
				...
				KePart kePa = (KePart) CommonUtils.getObject(oid);

				//pre.setLatest(false);
				//pre = (KePart) PersistenceHelper.manager.modify(pre);

				//KePart latest = KePart.newKePart();
				kePa.setOwnership(CommonUtils.sessionOwner());
				kePa.setNote(note);
				kePa.setState(dto.getState());
				PersistenceHelper.manager.modify(kePa);

				if (!StringUtils.isNull(cacheId)) {
					CommonContentHelper.manager.clear(kePa);
					ApplicationData dd = ApplicationData.newApplicationData(kePa);
					File vault = CommonContentHelper.manager.getFileFromCacheId(cacheId);
					dd.setRole(ContentRoleType.PRIMARY);
					dd = (ApplicationData) ContentServerHelper.service.updateContent(kePa, dd, vault.getPath());
				}
			}
