...
public class StandardWorkspaceService extends StandardManager implements WorkspaceService {
	...
	public void register(Persistable persistable, ArrayList<Map<String, String>> agreeRows,
		...
		try {
			...
			for (Map<String, String> approval : approvalRows) {
				...
				System.out.println("결재 라인 순서가?" + wtuser.getFullName());
	...
	@Override
	public void approvalReassign(Persistable beforePer, Persistable afterPer) throws Exception {
		Transaction trs = new Transaction();
		try {
			trs.start();

			ApprovalMaster appMaster = WorkspaceHelper.manager.getMaster(beforePer);

			appMaster.setPersist(appMaster);

			PersistenceHelper.manager.modify(appMaster);
	...
	public void _unagree(Map<String, Object> params) throws Exception {
		...
		try {
			...
			for (ApprovalLine l : list) {
				String s = CommonUtils.sessionUser().getFullName() + " 반려로 인한 모든 결재가 반려 처리 되엇습니다.";
				l.setDescription(s);
				l.setCompleteTime(completeTime);
				l.setState(WorkspaceHelper.STATE_AGREE_REJECT);
				l = (ApprovalLine) PersistenceHelper.manager.modify(l);
			}
	...
	private void setApprovalContractState(ApprovalContract contract) throws Exception {
		...
		while (result.hasMoreElements()) {
			...
			if (per instanceof WTPart) {
				...
				State s = State.toState("APPROVED");
				LifeCycleHelper.service.setLifeCycleState(latest, s);
				...
				NumberRule numberRule = (NumberRule) per;
				numberRule.setState(Constants.State.COMPLETE);
				PersistenceHelper.manager.modify(numberRule);
			}
		}
	}
...
//			ErpHelper.manager.postSendToErp(mm.getPersistInfo().getObjectIdentifier().getStringValue());
	...
	public void _reject(Map<String, Object> params) throws Exception {
		...
		try {
			...
			// 반려시 모든 결재도 반려되게 요청사항.. 완료일이 없는것들 전부 반려 시킨다
			ArrayList<ApprovalLine> list = WorkspaceHelper.manager.getAllLines(master);
			for (ApprovalLine aline : list) {
				if (aline.getCompleteTime() == null) {
					String name = CommonUtils.sessionUser().getFullName();
					String s = name + " 사용자의 결재 반려로 인한 일괄 반려 처리된 결재라인";
					aline.setDescription(s);
					aline.setCompleteTime(completeTime);
					aline.setCompleteUserID(CommonUtils.sessionUser().getName());

					if (aline.getType().equals(WorkspaceHelper.AGREE_LINE)) {
						aline.setState(WorkspaceHelper.STATE_AGREE_REJECT);
					} else if (aline.getType().equals(WorkspaceHelper.APPROVAL_LINE)) {
						aline.setState(WorkspaceHelper.STATE_APPROVAL_REJECT);
					} else if (aline.getType().equals(WorkspaceHelper.RECEIVE_LINE)) {
						aline.setState(WorkspaceHelper.STATE_APPROVAL_REJECT);
					}
