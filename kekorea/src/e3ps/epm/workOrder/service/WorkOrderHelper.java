...
public class WorkOrderHelper {
	...
	public Map<String, Object> list(Map<String, Object> params) throws Exception {
		...
		while (result.hasMoreElements()) {
			...
			WorkOrder wOrder = (WorkOrder) obj[0];
			...
			QuerySpecUtils.toEqualsAnd(_query, _idx_link, WorkOrderProjectLink.class, "roleAObjectRef.key.id", wOrder);
...
//			QuerySpecUtils.toLikeAnd(_query, _idx_p, Project.class, Project.KEK_NUMBER, kekNumber);
			...
			if (!StringUtils.isNull(kekNumber)) {
				...
				if (s.length > 0) {
					for (int k = 0; k < s.length; k++) {
						...
						_query.appendWhere(sc, new int[] { _idx_p });
					}
				}
			...
			node.put("oid", wOrder.getPersistInfo().getObjectIdentifier().getStringValue());
			node.put("name", wOrder.getName());
			node.put("number", wOrder.getNumber());
...
//				node.put("pdate_txt", dto.getPdate_txt());
				...
				if (!"".equals(reKekNumber)) {
					reKekNumber += "," + dto.getKekNumber();
				} else {
					reKekNumber = dto.getKekNumber();
				}

				if (!"".equals(reKeNumber)) {
					reKeNumber += "," + dto.getKeNumber();
				} else {
					reKeNumber = dto.getKeNumber();
				}

			}
			node.put("kekNumber", reKekNumber);
			node.put("keNumber", reKeNumber);
...
//		QuerySpecUtils.toIBALikeAnd(first, EPMDocument.class, idx1, "DWG_NO", number);
		...
		QuerySpecUtils.toEqualsAnd(second, _idx1, EPMDocument.class, EPMDocument.DOC_TYPE, "CADDRAWING");
//		QuerySpecUtils.toIBALikeAnd(second, EPMDocument.class, _idx1, "DWG_No", number);
		QuerySpecUtils.queryLikeNumber(second, EPMDocument.class, _idx1, number);
		QueryResult rs = PersistenceHelper.manager.find(second);
		System.out.println("second = " + rs.size());
		if (rs.hasMoreElements()) {
			Object[] obj = (Object[]) rs.nextElement();
			EPMDocument epm = (EPMDocument) obj[0];
			map.put("number", number);
			map.put("name", IBAUtils.getStringValue(epm, "TITLE1") + " " + IBAUtils.getStringValue(epm, "TITLE2"));
			map.put("rev", Integer.parseInt(epm.getVersionIdentifier().getSeries().getValue()));
			map.put("lotNo", "");
			map.put("current", epm.getVersionIdentifier().getSeries().getValue());
			map.put("ok", true);
...
//			sheet.setDefaultRowHeight((short) 22);
			...
			for (int j = start; j < end; j++) {
				...
				if ((list.size() - 1) >= j) {
					...
					if (per instanceof KeDrawing) {
						...
						if (m.getAuthoringApplication().toString().equals("ACAD")) {
							name = IBAUtils.getStringValue(epm, "TITLE1") + " "
									+ IBAUtils.getStringValue(epm, "TITLE2");
						} else {
							name = IBAUtils.getStringValue(epm, "NAME_OF_PARTS");
						}

						number = IBAUtils.getStringValue(epm, "DWG_NO");
						current = epm.getVersionIdentifier().getSeries().getValue();
						lotNo = String.valueOf(data.getLotNo());
					}
			...
			cell.setCellStyle(dataHeaderStyle);
			// cell.setCellValue("ALL DRAWING");
			cell.setCellValue(workOrder.getName());
			cell = row.createCell(15);
			cell.setCellStyle(dataHeaderStyle);
			cell = row.createCell(16);
			cell.setCellStyle(dataHeaderStyle);
...
//					map.put("primary", AUIGridUtils.primaryTemplate(keDrawing)); // pdf...
	 ...
	 * 이전버전의 도면 일람표 가져오기
	 */
	public WorkOrder pre(WorkOrder workOrder) throws Exception {
		String number = workOrder.getNumber();
		QuerySpec query = new QuerySpec();
		int idx = query.appendClassList(WorkOrder.class, true);
		QuerySpecUtils.toEquals(query, idx, WorkOrder.class, WorkOrder.NUMBER, number);
		QuerySpecUtils.toEqualsAnd(query, idx, WorkOrder.class, WorkOrder.VERSION, workOrder.getVersion() - 1);
		QueryResult result = PersistenceHelper.manager.find(query);
		if (result.hasMoreElements()) {
			Object[] obj = (Object[]) result.nextElement();
			WorkOrder prev = (WorkOrder) obj[0];
			return prev;
		}
