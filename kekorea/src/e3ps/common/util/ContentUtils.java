...
public class ContentUtils {
	 ...
	 * OID로 주 첨부 파일 내용들 가져오는 함수
	 */
	public static Map<String, Object> getThumbnail(String oid) throws Exception {
		ReferenceFactory rf = new ReferenceFactory();
		ContentHolder holder = (ContentHolder) rf.getReference(oid).getObject();
		return getThumbnail(holder);
	}

	/**
	 * ContentHolder 객체로 주 첨부 파일 내용들 가져오는 함수
	 */
	public static Map<String, Object> getThumbnail(ContentHolder holder) throws Exception {
		Map<String, Object> primary = null;
	...
	public static String getPreViewBase64(ContentHolder holder) throws Exception {
		...
		if (result.size() == 0) {
			Representable rep = PublishUtils.findRepresentable(holder);
			System.out.println("rep=" + rep);
			if (rep != null) {
				QueryResult rs = ContentHelper.service.getContentsByRole(rep, ContentRoleType.THUMBNAIL);
				if (rs.hasMoreElements()) {
					ContentItem item = (ContentItem) rs.nextElement();
					ApplicationData data = (ApplicationData) item;
					String ext = FileUtil.getExtension(data.getFileName());
					InputStream is = ContentServerHelper.service.findLocalContentStream(data);
					File tempFile = File.createTempFile(String.valueOf(is.hashCode()), ".tmp");
